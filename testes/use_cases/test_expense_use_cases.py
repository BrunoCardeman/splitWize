...
def _make_expense(exp_id, desc, amount, paid_by, participants):
    return Expense(
        id=exp_id,
        description=desc,
        amount=amount,
...
class TestCreateExpenseUseCase:
    ...
    def _make_use_case(self, payer=None, participants_map=None):
        expense_repo = MagicMock()
        expense_repo.save.side_effect = lambda e: Expense(
            id=1, description=e.description, amount=e.amount,
            paid_by_user_id=e.paid_by_user_id, participant_ids=e.participant_ids,
        )
        user_repo = MagicMock()
        ...
        return CreateExpenseUseCase(expense_repo, user_repo)
    ...
    def test_create_expense_negative_amount_raises(self):
        ...
        with pytest.raises(ValueError):
            use_case.execute("Jantar", -10.0, 1, [1])
    ...
    def test_create_expense_no_participants_raises(self):
        ...
        use_case = self._make_use_case(participants_map={1: payer})
        with pytest.raises(ValueError):
            use_case.execute("Jantar", 60.0, 1, [])
...
class TestSummaryUseCase:
    ...
    def test_summary_single_expense(self):
        ...
        """
        expense_repo = MagicMock()
        expense_repo.find_all.return_value = [
            _make_expense(1, "Jantar", 60.0, paid_by=1, participants=[1, 2])
        ]
        user_repo = MagicMock()
        user_repo.find_by_id.side_effect = lambda uid: {
            1: _make_user(1, "Ana"),
        ...
        debts = SummaryUseCase(expense_repo, user_repo).execute()
        assert len(debts) == 1
    ...
    def test_summary_balanced(self):
        ...
        expense_repo.find_all.return_value = [
            _make_expense(1, "Jantar", 60.0, paid_by=1, participants=[1, 2]),
            _make_expense(2, "Cinema", 60.0, paid_by=2, participants=[1, 2]),
        ]
        user_repo = MagicMock()
        debts = SummaryUseCase(expense_repo, user_repo).execute()
        assert debts == []